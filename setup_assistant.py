...
try:
    from dotenv import (
        load_dotenv,
    )  # Attempt to import a commonly used function from dotenv

    # If this succeeds, dotenv is already installed and available.
except ModuleNotFoundError:
    print(
        "Initial import of 'dotenv' failed. Module 'python-dotenv' appears to be missing."
    )
    ...
    try:
        ...
        print(
            "Please ensure 'pip' is working and then try installing it manually: pip install python-dotenv"
        )
        sys.exit(
            1
        )  # Exit if installation fails, as it's a critical dependency for setup.
    except ImportError:
        print("ERROR: Failed to import 'dotenv' even after attempting installation.")
        print(
            "This is unexpected. Please ensure 'python-dotenv' is correctly installed and then re-run the script."
        )
        sys.exit(1)
    except (
        FileNotFoundError
    ):  # This typically means sys.executable or pip itself wasn't found.
        print(
            "ERROR: Python executable or pip command not found. Ensure Python and pip are installed and in your system's PATH."
        )
        sys.exit(1)
...
def load_checkpoints():
    if os.path.exists(SETUP_CHECKPOINTS_PATH):
        try:
            ...
            print(
                f"Warning: Could not decode {SETUP_CHECKPOINTS_PATH}. File might be corrupted. Starting with fresh checkpoints."
            )
        except Exception as e:
            print(
                f"Warning: Error loading {SETUP_CHECKPOINTS_PATH}: {e}. Starting with fresh checkpoints."
            )
...
def main():
    ...
    # Define lazy import wrappers for functions that might depend on installed packages.
    # This defers the actual module import until the function is called,
    # ensuring dependencies (like PyTorch) are installed first by preceding setup steps.
    def lazy_create_dataset(dataset_path):
        from modules.dataset import create_dataset as actual_create_dataset

        actual_create_dataset(dataset_path)

    def lazy_fine_tune_model(dataset_path, model_save_path):
        from modules.model_training import fine_tune_model as actual_fine_tune_model

        actual_fine_tune_model(dataset_path, model_save_path)
    ...
    action_map = {
        ...
        "precise": (
            setup_precise,
            {"base_dir": BASE_DIR, "model_url": PRECISE_MODEL_URL},
        ),
        "picovoice_api_key": (
            setup_api_key,
            {
                "key_file_path": PICOVOICE_KEY_FILE_PATH,
                "service_name": "Picovoice",
                "prompt_message": "Enter Picovoice Access Key (or press Enter to skip): ",
            },
        ),
        "openweather_api_key": (
            setup_api_key,
            {
                "key_file_path": OPENWEATHER_API_KEY_FILE_PATH,
        ...
        "dataset": (
            ...
            {"dataset_path": INTENT_DATASET_CSV},
        ),  # Use centralized path
        "model_training": (
            lazy_fine_tune_model,  # Use the lazy wrapper
            {
                "dataset_path": INTENT_DATASET_CSV,
    ...
    while True:  # Main loop for the entire setup process
        ...
        for step_name in SETUP_STEPS:
            ...
            if not is_step_complete:
                ...
                if yes_no_prompt(
                    "This step is marked as complete. Do you want to redo it?"
                ):
                    run_this_step = True
                else:
                    print("Skipping step.")
            ...
            if run_this_step:
                while True:  # Loop for retrying a failed step
                    try:
                        print(
                            f"Running setup for {step_name.replace('_', ' ').title()}..."
                        )
                        func_to_call(**func_args)
                        checkpoints[step_name] = True
                        print(
                            f"Step '{step_name.replace('_', ' ').title()}' completed successfully."
                        )
                        # If device detection just ran, reload .env variables
                        if step_name == "device_detection":
                            print(
                                "Attempting to reload .env variables after device detection..."
                            )
                            load_dotenv(override=True)
                            print(
                                ".env variables reloaded. Subsequent operations will use these settings."
                            )
                        break  # Exit retry loop for this step
                    except Exception as e:
                        print(
                            f"Error during step '{step_name.replace('_', ' ').title()}': {e}"
                        )
                        checkpoints[step_name] = False

                        retry_choice = (
                            input(
                                "Step failed. Choose action: (r)etry, (s)kip this step for now, (e)xit setup: "
                            )
                            .strip()
                            .lower()
                        )
        ...
        if all_done_this_pass:
            ...
            if not yes_no_prompt(
                "Do you want to review or redo any steps? (If no, setup will finish)"
            ):
                break  # Exit main while loop, setup is finished
        else:
            print(
                "Some setup steps are not yet marked as complete or may have been skipped."
            )
            if not yes_no_prompt(
                "Do you want to go through the setup steps again (incomplete/skipped steps will be attempted, completed steps will offer a redo option)? (If no, setup will finish with current state)"
            ):
                break  # Exit main while loop

        # If continuing, ask about resetting all non-TTS steps
        if yes_no_prompt(
            "Do you want to mark ALL non-TTS steps as incomplete and start their setup over in the next pass?"
        ):
    ...
    print(
        "Well, this is the most you are supposed to do for voice assistants, who knows maybe there might be new typing areas added later."
    )
    ...
    try:
        ...
        from modules.config import (
            TTS_MODEL_NAME as DEFAULT_TTS_MODEL,
            TTS_SPEED_RATE as DEFAULT_TTS_SPEED,
            TTS_SAMPLERATE as DEFAULT_TTS_SAMPLERATE,
        )
        ...
        current_tts_samplerate = int(
            os.getenv("TTS_SAMPLERATE", str(DEFAULT_TTS_SAMPLERATE))
        )  # Samplerate is usually fixed but good to be consistent

        # --- Add this block to handle PyTorch 2.6+ weights_only=True issue for XTTS ---
        if "xtts" in current_tts_model.lower():
            print(
                "Setup Assistant (Final TTS): XTTS model detected, attempting to add safe globals for PyTorch 2.6+ compatibility."
            )
            safe_globals_to_add = []
            ...
            try:
                ...
                print(
                    "Setup Assistant (Final TTS): Identified TTS.tts.configs.xtts_config.XttsConfig for safe globals."
                )
            except ImportError:
                print(
                    "Setup Assistant (Final TTS) Warning: Could not import TTS.tts.configs.xtts_config.XttsConfig."
                )
            except Exception as e:
                print(
                    f"Setup Assistant (Final TTS) Warning: Error importing XttsConfig: {e}"
                )
            ...
            try:
                from TTS.tts.models.xtts import (
                    XttsAudioConfig,
                )  # Corrected import path based on typical structure

                safe_globals_to_add.append(XttsAudioConfig)
                print(
                    "Setup Assistant (Final TTS): Identified TTS.tts.models.xtts.XttsAudioConfig for safe globals."
                )
            except ImportError:
                print(
                    "Setup Assistant (Final TTS) Warning: Could not import TTS.tts.models.xtts.XttsAudioConfig."
                )
            except Exception as e:
                print(
                    f"Setup Assistant (Final TTS) Warning: Error importing XttsAudioConfig: {e}"
                )
            ...
            try:
                ...
                print(
                    "Setup Assistant (Final TTS): Identified TTS.config.shared_configs.BaseDatasetConfig for safe globals."
                )
            except ImportError:
                print(
                    "Setup Assistant (Final TTS) Warning: Could not import TTS.config.shared_configs.BaseDatasetConfig."
                )
            except Exception as e:
                print(
                    f"Setup Assistant (Final TTS) Warning: Error importing BaseDatasetConfig: {e}"
                )
            ...
            try:
                ...
                print(
                    "Setup Assistant (Final TTS): Identified TTS.tts.models.xtts.XttsArgs for safe globals."
                )
            except ImportError:
                print(
                    "Setup Assistant (Final TTS) Warning: Could not import TTS.tts.models.xtts.XttsArgs."
                )
            except Exception as e:
                print(
                    f"Setup Assistant (Final TTS) Warning: Error importing XttsArgs: {e}"
                )
            if safe_globals_to_add:
                try:
                    ...
                    Exception
                ) as e:  # Catch any exception during the add_safe_globals call
                    print(
                        f"Setup Assistant (Final TTS) Error: Could not apply safe globals: {e}"
                    )
            else:
                print(
                    "Setup Assistant (Final TTS) Warning: No XTTS specific classes found/imported to add to safe globals. XTTS models might fail to load if they require it."
                )
        # --- End block ---
        ...
        print(
            f"DEBUG (Final TTS): current_tts_model.lower() = '{current_tts_model.lower()}'"
        )
        print(
            f"DEBUG (Final TTS): 'xtts' in current_tts_model.lower() = {'xtts' in current_tts_model.lower()}"
        )

        tts_kwargs_final = {}
        if "xtts" in current_tts_model.lower():
            print(
                f"Final TTS: Model '{current_tts_model}' detected as an XTTS model. Checking for speaker WAV."
            )
            ...
            if os.path.exists(DEFAULT_SPEAKER_WAV_PATH):
                ...
                print(
                    f"Final TTS: Using default speaker WAV: {DEFAULT_SPEAKER_WAV_PATH} and language: {tts_kwargs_final['language']}"
                )
                print("DEBUG (Final TTS): DEFAULT_SPEAKER_WAV_PATH exists: True")
            else:
                print(
                    f"WARNING (Final TTS): Default speaker WAV for XTTS models not found at '{DEFAULT_SPEAKER_WAV_PATH}'."
                )
                print(
                    "The final message might not play correctly or use a default voice."
                )
                # Allow to proceed, it might use a default internal speaker if any, or fail as observed.

        audio_output = tts_instance_final.tts(
            text=final_tts_message, speed=current_tts_speed, **tts_kwargs_final
        )
        ...
        if (
            hasattr(tts_instance_final, "synthesizer")
            and tts_instance_final.synthesizer is not None
            and hasattr(tts_instance_final.synthesizer, "output_sample_rate")
        ):
            final_playback_samplerate = (
                tts_instance_final.synthesizer.output_sample_rate
            )
            print(
                f"Final TTS: Using model's output samplerate: {final_playback_samplerate}"
            )
