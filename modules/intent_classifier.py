...
INTENT_LABELS_MAP = {idx: label for idx, label in enumerate(unique_labels)} # Used as a fallback or reference
...
def initialize_intent_classifier():
    ...
    try:
        ...
        intent_model.load_state_dict(torch.load(model_weights_path, map_location=torch.device('cpu')))
        intent_model.eval() # Set the model to evaluation mode

        print("Intent Classifier (JointIntentSlotModel) initialized.")
    except Exception as e:
        print(f"Error initializing JointIntentSlotModel from {INTENT_MODEL_SAVE_PATH}: {e}")
        print("Ensure the model has been trained and saved correctly via setup_assistant.py.")
        raise
...
async def detect_intent_async(text: str) -> Tuple[str, Dict[str, Any]]:
    if intent_model is None or intent_tokenizer is None: # Updated check
        raise RuntimeError("Intent Classifier (JointIntentSlotModel) not initialized. Call initialize_intent_classifier() first.")
    ...
    inputs = intent_tokenizer(text, return_tensors="pt", truncation=True, padding=True, max_length=128) # Max length from training
    ...
    if confidence_score < CONFIDENCE_THRESHOLD:
        print(f"Intent score {confidence_score:.4f} below threshold {CONFIDENCE_THRESHOLD}. Falling back to {default_intent}.")
        return default_intent, entities

    # Use the model's id2label for intent (if available from config)
    detected_intent = intent_model.config.id2intent_label.get(predicted_intent_id, default_intent) \
        if hasattr(intent_model.config, 'id2intent_label') \
        else INTENT_LABELS_MAP.get(predicted_intent_id, default_intent)

    print(f"Detected intent: '{detected_intent}' with score {confidence_score:.4f} for text: '{text}'")
    ...
    id2slot_label = intent_model.config.id2slot_label if hasattr(intent_model.config, 'id2slot_label') else {}

    current_entity_value = ""
    current_entity_type = ""
    in_entity = False

    for token_idx, (token, predicted_slot_id_tensor) in enumerate(zip(tokens, predicted_slot_ids.squeeze())):
        if token in [intent_tokenizer.cls_token, intent_tokenizer.sep_token, intent_tokenizer.pad_token]:
            continue # Skip special tokens

        predicted_slot_label = id2slot_label.get(predicted_slot_id_tensor.item(), "O")

        if predicted_slot_label.startswith("B-"):
            if in_entity: # If already capturing an entity, store it before starting new one
                if current_entity_type and current_entity_value:
                    entities[current_entity_type] = entities.get(current_entity_type, "") + current_entity_value.strip() + " " # Append if multiple
                    entities[current_entity_type] = entities[current_entity_type].strip() # Clean up trailing space
            ...
            current_entity_value += token.replace("##", "") if "##" not in token else token.replace("##", "")
            in_entity = True
        elif predicted_slot_label.startswith("I-"):
            if in_entity and current_entity_type == predicted_slot_label[2:]:
                current_entity_value += " " + token.replace("##", "") if not token.startswith("##") else token.replace("##", "")
                ...
                if current_entity_type and current_entity_value:
                     entities[current_entity_type] = entities.get(current_entity_type, "") + current_entity_value.strip() + " "
    ...
    for key in entities:
        entities[key] = entities[key].strip().replace(" ##", "").replace("##", "") # General cleanup for subwords
